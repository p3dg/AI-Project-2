...
/**
 * @author Peter Galvin
 *   Node represents a node in a neural net
 */
public class Node {
    ...
    // calculate beta for a hidden layer
    public void calcBeta() {
    	m_beta = 0;
    	
    	for (Connection con: m_output_conn) {
    		double output = con.getToNode().getOutput();
    		double beta = con.getToNode().getBeta();
    		m_beta += (con.getWeight() * output * (1.0 - output) * beta);
    	}
    }
